...
import { MagnifyingGlass } from 'react-loader-spinner';
 ...
 function Update() {
    ...
    const getaEditData = async (param) => {
        ...
        try {
            // const q = query(colRefTwo, where("year", 'array-contains-any', [year]), where("grade", "==", grade), where("subject", "==", subject))
            const q = query(colRef, where("grade", "==", param.grade), where("subject", "==", param.subject), where("semester", "==", param.semester), where("year", "==", Number(param.year)))
            //const b = query(colRefTwo, where("grade", "==", '5'));   //where('brand', 'in', ['nike', 'adidas'])
            const querySnapshot = await getDocs(q);
            querySnapshot.forEach((doc) => {
                data.push({ ...doc.data(), id: doc.id })
            ...
            <div className=' justify-between w-screen'>
                ...
                <div className="flex justify-around">
                    ...
                    >
                        ...
                        < div className="text-xl md:my-0 mx-2 ">
                            <div className={styles.input_group}>
                                <input type="number" name="year" placeholder="Year" className={styles.input_text} onChange={handleChangeSearch} value={searchData.year} />
                            </div>
                        </div>
                        <div className={styles.input_group}>
                            <select
                    ...
                    <table className="min-w-full min-h-full ">
                        <thead className="border-b">
                            <tr>
                                ...
                                <th className="p-5 text-center">YEAR</th>
                        ...
                        <tbody>
                            {tableData.length > 0 && tableData.map((item, i) => (
                                ...
                                <tr key={i} className="border-b text-center" >
                                    ...
                                    <td className="p-5 text-center" onClick={()=>handlerowClick(item.id)}>
                                        {item.year}
                                    </td>
                                    <td className="p-5 text-center" onClick={()=>handlerowClick(item.id)}>{item.semester}</td>
